...
namespace TradeRiser.Core.Extensions
{
    ...
    public static class DataExtensions
    {
        ...
        /// <param name="ordinal">The ordinal.</param>
        /// <param name="defaultValue">The default value.</param>
        public static T Get<T>(this NpgsqlDataReader reader, int ordinal, T defaultValue)
        {
            T result = defaultValue;
            ...
            try
            {
                if (reader.FieldCount > ordinal)
                {
                    if (!reader.IsDBNull(ordinal))
                    {
                        ...
                        {
                            throw new NotImplementedException("Postgres exp");
                            //return (T)Convert.ChangeType(reader.GetSqlBinary(ordinal).Value, typeof(T), CultureInfo.CurrentCulture);
                        }
                        else if (typeof(T).BaseType == typeof(Enum))
                        {
        ...
        /// <param name="parameter">The parameter.</param>
        /// <param name="defaultValue">The default value.</param>
        public static T Value<T>(this NpgsqlParameter parameter, T defaultValue)
        {
            T result = defaultValue;
            if (parameter != null)
            {
