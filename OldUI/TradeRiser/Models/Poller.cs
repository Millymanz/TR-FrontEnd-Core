...
namespace TradeRiser.Models
{
    ...
    public static class Poller
    {
        ...
        static Dictionary<String, QueryService.SessionTransact> queryData = new Dictionary<string, QueryService.SessionTransact>();
        ...
        public static void HandleBroadcast(object sender, EventArgs e)
        {
            try
            {
               ...
               // String tempText = String.Format("{0} (from {1})", eventData.EventMessage.UserQuerySessionID, eventData.EventMessage.QueryID);

                var presentRender = new PresentationRenderer(eventData.EventMessage);

                PushNotification.Push(presentRender, eventData.EventMessage.UserQuerySessionID);
                
                //PushNotification.Push(presentRender, "Stock1");               
