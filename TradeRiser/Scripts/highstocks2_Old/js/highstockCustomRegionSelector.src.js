 ...
 *
 * License: www.chartRegionSelector.com/license
 */

// JSLint options:
/*global ChartRegionSelector, document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location, jQuery, $, console, each, grep */



selectedValuesY = new Array();
selectedValuesX = new Array();
var globalmousedown;
var globalChart;
var globalContainer;
var globalSeries;
...
var mousedownIndex;
var mousedownEndingIndex;
var mouseupIndex;

var globalGraphicalQueryStartPeriod;
var globalGraphicalQueryEndPeriod;
...
// The ChartRegionSelector namespace
win.ChartRegionSelector = win.ChartRegionSelector ? error(16, true) : {};
 ...
 * First, it deep merged arrays, which lead to workarounds in ChartRegionSelector. Second,
...
function error(code, stop) {
	var msg = 'ChartRegionSelector error #' + code + ': www.chartRegionSelector.com/errors/' + code;
	if (stop) {
		throw msg;
	} else if (win.console) {
		console.log(msg);
...
(function ($) {
	/**
	 * The default ChartRegionSelectorAdapter for jQuery
	 */
	win.ChartRegionSelectorAdapter = win.ChartRegionSelectorAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			// extend some methods to check for elem.attr, which means it is a ChartRegionSelector SVG object
			...
			/**
			 * Register ChartRegionSelector as a plugin in the respective framework
			 */
			$.fn.chartRegionSelector = function () {
				...
				if (options !== UNDEFINED) {
					...
					chart = new ChartRegionSelector[constr](options, args[1]);
					ret = this;
					/*jslint unused:true*/
				}
				...
				if (options === UNDEFINED) {
					ret = charts[attr(this[0], 'data-chartRegionSelector-chart')];
				}	
		...
		fireEvent: function (el, type, eventArguments, defaultFunction) {
			...
			// Remove warnings in Chrome when accessing layerX and layerY. Although ChartRegionSelector
...
// check for a custom ChartRegionSelectorAdapter defined prior to this file
var globalAdapter = win.ChartRegionSelectorAdapter,
...
// Utility functions. If the ChartRegionSelectorAdapter is not defined, adapter is an empty object
...
defaultOptions = {
	...
	global: {
		useUTC: true,
		canvasToolsURL: 'http://code.chartRegionSelector.com/stock/1.3.5/modules/canvas-tools.js',
		VMLRadialGradientURL: 'http://code.chartRegionSelector.com/stock/1.3.5/gfx/vml-radial-gradient.png'
	},
	chart: {
		//animation: true,
		//alignTicks: false,
	...
	credits: {
		enabled: true,
		text: 'ChartRegionSelector.com',
		href: 'http://www.chartRegionSelector.com',
		position: {
			align: 'right',
			x: -10,
			verticalAlign: 'bottom',
...
SVGRenderer.prototype = {
	...
	/**
	 * Create a group
	 * @param {String} name The group will be given a class name of 'chartRegionSelector-{name}'.
	...
	fontMetrics: function (fontSize) {
		...
		// Applies to the default font family. http://jsfiddle.net/chartRegionSelector/7xvn7/
 ...
 */
ChartRegionSelector.VMLElement = VMLElement = {
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		...
		// Test case: http://chartRegionSelector.com/tests/?file=text-rotation
		css(this.element, {
...
ChartRegionSelector.VMLRenderer = VMLRenderer = function () {
	this.init.apply(this, arguments);
};
...
if (useCanVG) {
	 ...
	 */
	ChartRegionSelector.CanVGRenderer = CanVGRenderer = function () {
...
Pointer.prototype = {
	...
	onContainerMouseDown: function (e) {
		...
		var chart = this.chart;
		if (chart.selectionMarker) {
		    chart.selectionMarker = chart.selectionMarker.destroy();
		}
		...
		globalmousedown = mouseDownX;
		globalChart = chart;

		index = e.chartX - chart.plotLeft + 10; // wtf?
		mousedownIndex = index;


		var teppf = document.getElementById('startDateTest');
		var tempstartDateDisplaty = document.getElementById('tempM');

	    //convert the event day to a date object
		var startticks = new Date(chart.series[0].tooltipPoints[mousedownIndex].x);
		...
		var mlo = days + ' : ' + hours + ' : ' + mins;

		//teppf.innerText = chart.series[0].xData[mousedownIndex];

		//teppf.innerText = dateFormat("%A, %b %e, %Y", chart.series[0].tooltipPoints[mousedownIndex].x);

		teppf.innerText = dateFormat("%b %e, %Y", chart.series[0].tooltipPoints[mousedownIndex].x);
		globalGraphicalQueryStartPeriod = chart.series[0].tooltipPoints[mousedownIndex].x;
        //----------------------//
	...
	onDocumentMouseMove: function (e) {
		...
		if (chartPosition && hoverSeries && !this.inClass(e.target, 'chartRegionSelector-tracker') &&
	...
	onContainerMouseMove: function (e) {
		...
		if (chart.mouseIsDown === 'mousedown') {
			...
			//-------Added Code Dennis--------//
			    ...
			    hasDragged = Math.sqrt(
                    Math.pow(e.chartX - chart.mouseDownX, 2) +
                    Math.pow(e.chartY - chart.mouseDownY, 2)
                );

			    var widthTemp = 0;

					if (hasDragged > 10) {
					    var clickedInside = chart.isInsidePlot(chart.mouseDownX - chart.plotLeft, chart.mouseDownY - chart.plotTop);

					    if (clickedInside) {
							if (!chart.selectionMarker) {
							    chart.selectionMarker = chart.renderer.rect(
									chart.plotLeft,
									chart.plotTop,
									chart.zoomHor ? 1 : chart.plotWidth,
									chart.zoomVert ? 1 : chart.plotHeight,
									0
								)
								.attr({
						...
						// adjust the width of the selection marker
					    //if (chart.selectionMarker && chart.zoomHor) {
					    if (chart.selectionMarker) {
					        var xSize = e.chartX - chart.mouseDownX;

					        widthTemp = mathAbs(xSize);

						    chart.selectionMarker.attr({
						        width: mathAbs(xSize),						       
						        x: chart.mouseDownX
								//x: (xSize > 0 ? 0 : xSize) + chart.mouseDownX
							});
						}
						// adjust the height of the selection marker
					    if (chart.selectionMarker && chart.zoomVert) {
						    var ySize = chart.chartY - chart.mouseDownY;
						    chart.selectionMarker.attr({
								height: mathAbs(ySize),
								y: (ySize > 0 ? 0 : ySize) + chart.mouseDownY
							});
						}

						// panning
					    if (clickedInside && !chart.selectionMarker && optionsChart.panning) {
							chart.pan(chartX);
						}
					}

					mousedownEndingIndex = mousedownIndex + widthTemp - 20;

					var endtt = document.getElementById('endDateTest');
					//endtt.innerText = chart.series[0].tooltipPoints[mousedownEndingIndex].x;

					endtt.innerText = dateFormat("%b %e, %Y", chart.series[0].tooltipPoints[mousedownEndingIndex].x);
					globalGraphicalQueryEndPeriod = chart.series[0].tooltipPoints[mousedownEndingIndex].x;
		...
		if ((this.inClass(e.target, 'chartRegionSelector-tracker') || 
...
Chart.prototype = {
	...
	getContainer: function () {
		var chart = this,
			...
			chartHeight,
			renderTo,
			indexAttrName = 'data-chartRegionSelector-chart',
			oldChartIndex,
			containerId;
	...
	destroy: function () {
		...
		chart.renderTo.removeAttribute('data-chartRegionSelector-chart');
...
Series.prototype = {
	 ...
	 * symbols through ChartRegionSelector.seriesTypes[type].prototype.drawLegendSymbols.
	 * 
	...
	setData: function (data, redraw) {
		...
		if (turboThreshold && dataLength > turboThreshold) { 
			...
			if (isNumber(firstPoint)) { // assume all points are numbers
				...
				error(12); // ChartRegionSelector expects configs to be numbers or arrays in turbo mode
			}
		} else {
			for (i = 0; i < dataLength; i++) {
	 ...
	 * unused points and optionally grouped in ChartRegionSelector Stock.
	 */
...
ChartRegionSelector.Scroller = Scroller;
...
ChartRegionSelector.RangeSelector = RangeSelector;
 ...
 */
ChartRegionSelector.StockChart = function (options, callback) {
	...
	seriesProto.init = function () {
		...
		if (xAxis && xAxis.options.ordinal && !xAxis.hasOrdinalExtension) {
			...
			xAxis.postProcessTickInterval = function (tickInterval) {
				// TODO: http://jsfiddle.net/chartRegionSelector/FQm4E/1/
...
function selectionEvent(e) {
    ...
    var tempplot = offset(globalContainer);

    var index = tempplot.left - globalplotLeft; // wtf?
    mousedownIndex = index;


    //document.getElementById('onmu').innerHTML = "(" + tempplot.left + " ," + tempplot.top + ") " + index;

    e = normalizeMouseEvent(e);

    var currentplotLeft = tempplot.left;

    var startIndex = e.chartX - globalmousedown - globalplotLeft

    //var startIndex = e.chartX - globalmousedown;
    var endIndex = e.chartX;

    //document.getElementById('selectionDetails').innerHTML =
    //document.getElementById('selectionDetails').innerHTML + "Entry<br/>";



    if (startIndex > 0 && endIndex < globalChart.series[0].tooltipPoints.length) {
        var point;
        for (var i = startIndex; i <= endIndex; i++) {
            //point = globalChart.series[0].tooltipPoints[i];
            point = globalSeries[i];
            selectedValuesY.push(point.y);

            var dObject = new Date(point.x);
            var month = dObject.getMonth();
            var dateStr = dObject.getDate() + "/" + month + "/" + dObject.getFullYear();


            document.getElementById('selectionDetails').innerHTML =
            document.getElementById('selectionDetails').innerHTML + " [" + dateStr + "," + point.y + "]";
        }
    }


    document.getElementById('selectionDetails').innerHTML =
    document.getElementById('selectionDetails').innerHTML + "Exit<br/>";
}
