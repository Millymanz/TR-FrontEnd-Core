 ...
 *
 * License: www.HighstockCustom.com/license
 */

// JSLint options:
/*global HighstockCustom, document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location, jQuery, $, console */
...
var UNDEFINED,
	...
	// check for a custom HighstockCustomAdapter defined prior to this file
	globalAdapter = win.HighstockCustomAdapter,
	adapter = globalAdapter || {},

	// Utility functions. If the HighstockCustomAdapter is not defined, adapter is an empty object
...
function error(code, stop) {
	var msg = 'HighstockCustom error #' + code + ': www.HighstockCustom.com/errors/' + code;
	if (stop) {
		throw msg;
	} else if (win.console) {
		console.log(msg);
 ...
 * HighstockCustom reverts to the built-in jQuery adapter.
 */
...
if (!globalAdapter && win.jQuery) {
	...
	// extend some methods to check for elem.attr, which means it is a HighstockCustom SVG object
...
defaultOptions = {
	...
	global: {
		useUTC: true,
		canvasToolsURL: 'http://code.HighstockCustom.com/stock/1.1.5/modules/canvas-tools.js'
	},
	chart: {
		//animation: true,
		//alignTicks: false,
	...
	credits: {
		enabled: true,
		text: 'HighstockCustom.com',
		href: 'http://www.HighstockCustom.com',
		position: {
			align: 'right',
			x: -10,
			verticalAlign: 'bottom',
		...
		if (elem.tagName === 'SPAN') {
			...
			if (currentTextTransform !== wrapper.cTT) { // do the calculations and DOM access only if properties changed
				...
				if (defined(rotation)) {
					...
					// Test case: http://HighstockCustom.com/tests/?file=text-rotation
					css(elem, {
...
SVGRenderer.prototype = {
	...
	/**
	 * Create a group
	 * @param {String} name The group will be given a class name of 'HighstockCustom-{name}'.
	...
	fontMetrics: function (fontSize) {
		...
		// Applies to the default font family. http://jsfiddle.net/HighstockCustom/7xvn7/
...
function Chart(userOptions, callback) {
	...
	function MouseTracker(options) {
		...
		//den code	
		document.getElementById('onmmm').innerHTML ="("+ points.x +" ,"+ points.y + ") " + index;
		...
		function drop() {
			if (selectionMarker) {
				...
				if (hasDragged) {
					...
					//*fireEvent(chart, 'mouseup', selectionDataEvent);
			...
			//*removeEvent(chart, 'mouseup', testmethodevent);
		...
		function setDOMEvents() {
			...
			container.onmousedown = function (e) {
				...
				document.getElementById('onmd').innerHTML ="("+ e.chartX +" ,"+ e.chartY + ") " + index;

				
				
				
				
				//addEvent(container, 'mouseup', selectionDataEvent );
				//addEvent(chart, 'mouseup', testmethodevent);
				...
				{
					selectionMarker = selectionMarker.destroy();
					selectedValuesY.splice(0, selectedValuesY.length);
					
					selectedValuesX.splice(0, selectedValuesX.length);
				}
			};
			...
			// issue #295, dragging not always working in Firefox
				...
				if (globalmousedown <  e.chartX)
				{
						//den added
				
				index = inverted ? e.chartY : e.chartX - plotLeft; // wtf?
				mouseupIndex = index;
				document.getElementById('onmu').innerHTML ="("+ e.chartX +" ,"+ e.chartY + ") " + index +" "+  globalSeries.length;
		...
		document.getElementById('selectionDetails').innerHTML +"Entry<br/>";*/
			...
			for (var i = mousedownIndex; i <= mouseupIndex; i++)
			{
				...
				{
					var bDuplication = false;
					var count = tempPoints.length;
					for (var d = 0; d < count; d++)
					{
					    var variableTemp = tempPoints[d];
						if (variableTemp.x == point.x)
						{
							bDuplication = true;
						}
					}
			...
			//selected y values
				...
				{
				var currentPoint = tempPoints[l];
			
			
				var dObject = new Date(currentPoint.x);
				var month = dObject.getMonth() + 1;
				var dateStr = dObject.getDate() +"/"+ month +"/"+ dObject.getFullYear();
				
					/*document.getElementById('selectionDetails').innerHTML = 
					document.getElementById('selectionDetails').innerHTML +" ["+ dateStr + "," + currentPoint.y+"] ";*/
					
					selectedValuesY.push(currentPoint.y);
				}
				
				
				
					/*document.getElementById('selectionDetails').innerHTML = 
		document.getElementById('selectionDetails').innerHTML + " Total plots" + tempPoints.length +"  Exit <br/>";	*/
	...
	function firstRender() {
		...
		if (HighstockCustom.RangeSelector && options.rangeSelector.enabled) {
			chart.rangeSelector = new HighstockCustom.RangeSelector(chart);
			
			//den code
			//chart.rangeSelector.css('border', 'solid 5px red');
		...
		if (HighstockCustom.Scroller && (options.navigator.enabled || options.scrollbar.enabled)) {
			chart.scroller = new HighstockCustom.Scroller(chart);
		}
...
Series.prototype = {
	...
	setData: function (data, redraw) {
		...
		if (dataLength > turboThreshold) {
			...
			if (isNumber(firstPoint)) { // assume all points are numbers
				...
				error(12); // HighstockCustom expects configs to be numbers or arrays in turbo mode
			}*/
		} else {
			for (i = 0; i < dataLength; i++) {
				pt = { series: series };
	 ...
	 * unused points and optionally grouped in HighstockCustom Stock.
	 */
 ...
 * @param {Object} chart
 */
HighstockCustom.Scroller = function (chart) {
 ...
 * @param {Object} chart
 */
HighstockCustom.RangeSelector = function (chart) {
	var renderer = chart.renderer,
		rendered,
	...
	/*
		...
		//den code
		boxSpanElements[name].style.color = "blue";
		boxSpanElements[name].style.position = "absolute";
		boxSpanElements[name].style.top = "12px";
		boxSpanElements[name].style.left = "12px";
		boxSpanElements[name].style.zIndex = "4060";
		...
		input.style.left = "1050px";
		input.style.zIndex = "4060";
 ...
 */
HighstockCustom.StockChart = function (options, callback) {
	...
	seriesProto.init = function () {
		...
		if (xAxis && xAxis.options.ordinal && !xAxis.hasOrdinalExtension) {
			...
			xAxis.postProcessTickInterval = function (tickInterval) {
				// TODO: http://jsfiddle.net/HighstockCustom/FQm4E/1/
...
var ePos,
			...
			// get mouse position
			chartPosition = offset(globalContainer);
			chartPosLeft = chartPosition.left;
			chartPosTop = chartPosition.top;
...
{
	//var currentplotLeft = getPosition(document.getElementById('highstockCustom'));
	...
	var checkme = this;
        //*var index = e.pageX - currentplotLeft;
		//var index = e.pageX - globalmousedown.;
		//starting index ending index 
		
		//*var startIndex = currentplotLeft - globalmousedown;
		//*var endIndex = e.pageX - currentplotLeft;
