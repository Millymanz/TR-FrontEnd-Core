...
/**
 * @license HighchartsCustom JS v2.2.0 (2012-02-16)
 *
...
// JSLint options:
/*global HighchartsCustom, document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location, jQuery, $, console */
...
// The HighchartsCustom namespace
win.HighchartsCustom = {};
...
function error(code, stop) {
	var msg = 'HighchartsCustom error #' + code + ': www.highcharts.com/errors/' + code;
	if (stop) {
		throw msg;
	} else if (win.console) {
		console.log(msg);
 ...
 * HighchartsCustom reverts to the built-in jQuery adapter.
 */
...
if (!globalAdapter && win.jQuery) {
	...
	// extend some methods to check for elem.attr, which means it is a HighchartsCustom SVG object
...
function Chart(options, callback) {
	...
	function MouseTracker(options) {
		...
		function setDOMEvents() {
			...
			container.onclick = function (e) {
				...
				var criteriaDialog = document.getElementById('criteriaDialog');
	...
	function firstRender() {
		...
		if (HighchartsCustom.RangeSelector && options.rangeSelector.enabled) {
			chart.rangeSelector = new HighchartsCustom.RangeSelector(chart);
		}
		...
		if (HighchartsCustom.Scroller && (options.navigator.enabled || options.scrollbar.enabled)) {
			chart.scroller = new HighchartsCustom.Scroller(chart);
		}
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		extend(series, {
			index: index,
			name: options.name, //* den code|| 'Series ' + (index + 1),
			state: NORMAL_STATE,
			pointAttr: {},
	...
	setData: function (data, redraw) {
		...
		if (dataLength > turboThreshold) {
			...
			if (isNumber(firstPoint)) { // assume all points are numbers
				...
				error(12); // HighchartsCustom expects configs to be numbers or arrays in turbo mode
			}*/
		} else {
			for (i = 0; i < dataLength; i++) {
				pt = { series: series };
	 ...
	 * unused points and optionally grouped in HighchartsCustom Stock.
	 */
